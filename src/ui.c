...
void display_file_info(const struct stat *st, const char *perms,
                       const struct passwd *pw, const struct group *gr,
                       const char *date, double size, const char *un,
                       const char *filename) {
  if (!st || !perms || !date || !un) {
    return;
  }
  ...
  snprintf(info_buffer, sizeof(info_buffer), "%ld %s %s %.0f%s %s",
           st->st_nlink, pw ? pw->pw_name : "?", gr ? gr->gr_name : "?", size,
           un, date);
  ...
  if (selected == 0) {
    snprintf(position_info, sizeof(position_info), "%s %d/%d", position,
             selected + 1, file_count);
  } else if (selected == file_count - 1) {
    snprintf(position_info, sizeof(position_info), "%s %d/%d", position,
             selected + 1, file_count);
  } else {
    snprintf(position_info, sizeof(position_info), "%d%% %d/%d", percentage,
             selected + 1, file_count);
  }
    ...
    if (len != -1) {
      ...
      if (available_space > 0) {
        ...
        if (strlen(link_target) > available_space) {
          snprintf(display_target, available_space + 1, "%.*s...",
                   available_space - 3, link_target);
        } else {
  ...
  int right_offset = COLS - (int)strlen(position_info) - 1;
  if (right_offset > 0) {
    mvprintw(LINES - 1, right_offset, "%s", position_info);
  }
}
  ...
  *dst = '\0';
  ...
  for (int i = 0; i < height && i + offset < file_count; i++) {
    ...
    if (name_len > max_name_width) {
      snprintf(display_name, sizeof(display_name), "%.*s~", max_name_width - 1,
               name);
    } else {
