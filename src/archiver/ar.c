...
void extract(const char* filename, const char* output_dir, int show_contents) {
    ...
    while ((r = archive_read_next_header(a, &entry)) == ARCHIVE_OK) {
        ...
        if (is_pure_gz) {
            ...
            if (path) strncpy(clean_path, path, sizeof(clean_path) - 1);
        ...
        if (output_dir && output_dir[0] != '\0') {
            ...
            if (outlen > 0 && clean_output[outlen - 1] == '/') clean_output[outlen - 1] = '\0';
...
void list(const char* filename) {
    ...
    while ((r = archive_read_next_header(a, &entry)) == ARCHIVE_OK) {
        ...
        printf("%s %10" PRIdMAX " %s %s\n", archive_entry_strmode(entry),
               (intmax_t)archive_entry_size(entry), timestr_trimmed, path ? path : "");
...
int main(int argc, char** argv) {
    ...
    if (!archive_path) {
        fprintf(stderr, "No archive specified. Use --help for usage information.\n");
        return 1;
    }
    ...
    if (mode == 1) {
        ...
        fprintf(stderr, "Invalid mode specified. Use --help for usage information.\n");
        return 1;
    }
