...
void rm(const char *path) {
  ...
  if (lstat(path, &st) != 0) {
    mvprintw(LINES - 1, 0, "Error: Cannot access '%s' - %s", path,
             strerror(errno));
    refresh();
    getch();
    return;
  ...
  if (S_ISLNK(st.st_mode)) {
    if (unlink(path) != 0) {
      mvprintw(LINES - 1, 0, "Error: Cannot remove symlink '%s' - %s", path,
               strerror(errno));
      refresh();
      getch();
    }
  ...
  if (S_ISDIR(st.st_mode)) {
    ...
    if (!dir) {
      mvprintw(LINES - 1, 0, "Error: Cannot open directory '%s' - %s", path,
               strerror(errno));
      refresh();
      getch();
      return;
    ...
    if (rmdir(path) != 0) {
      if (errno == EACCES) {
        if (chmod(path, 0700) == 0) {
          if (rmdir(path) != 0) {
            mvprintw(LINES - 1, 0, "Error: Cannot remove directory '%s' - %s",
                     path, strerror(errno));
            refresh();
            getch();
          }
          ...
          mvprintw(LINES - 1, 0,
                   "Error: Cannot change permissions for '%s' - %s", path,
                   strerror(errno));
          refresh();
          getch();
        }
      } else {
        mvprintw(LINES - 1, 0, "Error: Cannot remove directory '%s' - %s", path,
                 strerror(errno));
        refresh();
        getch();
      }
    ...
    if (remove(path) != 0) {
      if (errno == EACCES) {
        if (chmod(path, 0600) == 0) {
          if (remove(path) != 0) {
            mvprintw(LINES - 1, 0, "Error: Cannot remove file '%s' - %s", path,
                     strerror(errno));
            refresh();
            getch();
          }
...
void open_file(const char *filename) {
  ...
  pid_t pid = fork();
  if (pid == 0) {
    if (strncmp(mime_type, "text/", 5) == 0 || strstr(mime_type, "json") ||
        strstr(mime_type, "xml") || strstr(mime_type, "script")) {
      execlp(config.default_viewer, config.default_viewer, full_path,
             (char *)NULL);
    } else {
      setsid();
      if (strncmp(mime_type, "image/", 6) == 0) {
        execlp(config.image_viewer, config.image_viewer, full_path,
               (char *)NULL);
      } else {
...
int confrim_delete(const char *filename) {
  ...
  mvprintw(LINES - 2, 0, "Confirm deletion of: %s (y/N): ", filename);
  refresh();
...
void goto_cmd(int next_char) {
  ...
  case 'k': 
    ...
    break;
  case 'f':
    strcpy(target_path, "/usr/local/");
    break;
  case 'w': 
    strcpy(target_path, "/usr/bin/");
    break;
